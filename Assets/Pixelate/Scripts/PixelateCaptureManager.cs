...
public class PixelateCaptureManager : MonoBehaviour
{
    ...
    public IEnumerator CaptureFrame(Action<Texture2D, Texture2D, bool, bool, Vector2, Vector2> onComplete)
    {
        if (_target == null)
        {
            Debug.LogWarning("CaptureCamera should be set before capturing animation!");
        ...
        try
        {
            ...
            onComplete.Invoke(diffuseMap, normalMap, _createNormalMap, _useAnimation, _cellSize, new Vector2(0.5f, 0.5f));
        }
        finally
        {
            QualitySettings.renderPipeline = defaultQualityPipeline;
    ...
    public IEnumerator CaptureAnimation(Action<Texture2D, Texture2D, bool, bool, Vector2, Vector2> onComplete)
    {
        if (_sourceClip == null || _target == null)
        {
            Debug.LogWarning("CaptureCamera should be set before capturing animation!");
        ...
        try
        {
            ...
            onComplete.Invoke(diffuseMap, normalMap, _createNormalMap, _useAnimation, _cellSize, _pivot);
        }
        finally
        {
            QualitySettings.renderPipeline = defaultQualityPipeline;
